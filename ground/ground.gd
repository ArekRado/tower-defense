...
@onready var bottom_static_body2D: StaticBody2D = $BottomEdges
@onready var top_static_body2D: StaticBody2D = $TopEdges
...
func _ready() -> void:
	...
	create_colliders_for_cells(bottom_cells, get_bottom_edge_position, bottom_static_body2D)
	create_colliders_for_cells(top_cells, get_top_edge_position, top_static_body2D)
...
func get_bottom_edge_position(cell_position: Vector2) -> Array[Vector2]:
	return [
		cell_position + Vector2(0, tile_set.tile_size.y),
		cell_position + Vector2(tile_set.tile_size.x, tile_set.tile_size.y)
	]

func get_top_edge_position(cell_position: Vector2) -> Array[Vector2]:
	return [
		cell_position + Vector2(0, 0),
		cell_position + Vector2(tile_set.tile_size.x, 0)
	]

func create_colliders_for_cells(cells: Array[Vector2i], get_edge_position: Callable, static_body2D: StaticBody2D) -> void:
